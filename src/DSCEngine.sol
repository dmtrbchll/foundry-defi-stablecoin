 ...
 * all collateral be <= the $ backed value of all the DSC tokens.
 *
 * @notice This contract is the core of the DSC System. It handles all the logic for minting and
 * reedeeming DSC tokens, as well as depositing & withdrawing collateral.
...
contract DSCEngine is ReentrancyGuard {
     ...
     * @param user The use who has broken the health factor. Their _healthFactor should be
    ...
    function liquidate(address collateral, address user, uint256 debtToCover)
        ...
        nonReentrant
    {
        // need to check health factor of user to liquidate
        ...
        // $100 DSC == ??? ETH
        // 0.05 ETH
        // tokenAmountFromDebtCovered is WETH or WBTC in our case
        uint256 tokenAmountFromDebtCovered = getTokenAmountFromUsd(collateral, debtToCover);
        // And give them a 10% bonus
        // So we are giving the liquidator 110 of WETH for 100 DSC
        // We should implement a feature to liqiudate in the event the protocol is insolvent
    ...
    // Get the value in USD for some token and the respective amount of that token
    ...
    function getTokenAmountFromUsd(address token, uint256 usdAmountInWei) public view returns (uint256) {
        // price of ETH (token)
        // Dollar/ETH -> ?? ETH
        // 1 ETH = $2000, how much ETH is $1000 => 0.5 ETH
        AggregatorV3Interface priceFeed = AggregatorV3Interface(s_priceFeeds[token]);
        (, int256 price,,,) = priceFeed.staleCheckLatestRoundData();
        // ($10 * 1e18) / ($2000e8 * 1e10)
